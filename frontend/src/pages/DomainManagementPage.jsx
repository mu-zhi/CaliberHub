...
export function DomainManagementPage() {
  ...
  const loadDomains = useCallback(async (nextSelectedId = selectedIdRef.current) => {
    ...
    try {
      ...
      setMeta(`已加载 ${nextRows.length} 个业务域。`);
    } catch (err) {
      setRows([]);
      setSelectedId(null);
      setForm(createEmptyForm());
      setError(err.message || "加载业务域失败");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  function selectDomain(domain) {
    ...
    setError("");
    setMeta(`已切换到业务域：${domain.domainName || domain.domainCode || "-"}`);
  }
  ...
  async function saveDomain() {
    ...
    try {
      ...
      const saved = form.id
        ? await apiRequest(`/domains/${form.id}`, { method: "PUT", body: payload })
        : await apiRequest("/domains", { method: "POST", body: payload });
      const savedId = saved?.id || null;
      setMeta(`${form.id ? "更新" : "创建"}成功：${saved?.domainName || saved?.domainCode || "-"}`);
      await loadDomains(savedId);
    } catch (err) {
      setError(err.message || "保存业务域失败");
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="domain-workspace">
        <aside className="panel-block domain-list">
          <div className="row form-row">
            <label htmlFor="domainKeyword">业务域筛选</label>
            <input
              id="domainKeyword"
              name="domainKeyword"
              autoComplete="off"
        ...
        <section className="panel-block domain-editor">
          <div className="domain-editor-head">
            <h3>{form.id ? `编辑业务域 #${form.id}` : "新建业务域"}</h3>
            <p className="subtle-note">业务域编码将自动转为大写保存。</p>
          </div>
          <div className="search-grid">
            <div className="row form-row">
              <label htmlFor="domainCode">业务域编码</label>
              <input
                id="domainCode"
                name="domainCode"
                autoComplete="off"
            ...
            <div className="row form-row">
              <label htmlFor="domainName">业务域名称</label>
              <input
                id="domainName"
                name="domainName"
                autoComplete="off"
          ...
          <div className="actions">
            <button className="btn btn-primary" type="button" onClick={saveDomain} disabled={saving}>
              {saving ? "保存中…" : (form.id ? "更新业务域" : "创建业务域")}
            </button>
