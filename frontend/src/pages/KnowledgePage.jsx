...
export function KnowledgePage({ preset = "import" }) {
  ...
  const loadDomains = useCallback(async () => {
    ...
    try {
      ...
      const nextDomains = Array.isArray(result) ? result : [];
      setDomains(nextDomains);
      if (nextDomains.length === 0) {
        setDomainLoadError("暂无业务域，请先在业务域管理维护。");
      }
      return nextDomains;
    } catch (_) {
      setDomains([]);
      setDomainLoadError("业务域加载失败，请稍后重试。");
      return [];
    } finally {
  ...
  async function handleReloadDomains() {
    ...
    if (nextDomains.length > 0) {
      setPreprocessMeta(`业务域刷新完成，共 ${nextDomains.length} 个可选项。`);
      setError("");
      return;
    }
    setError("当前无可选业务域，请点击“去业务域管理”先创建业务域。");
  }
  ...
  async function quickCreateDefaultDomain() {
    ...
    try {
      const payload = {
        ...
        domainName: "未分类业务域",
        domainOverview: "用于导入阶段临时归集未明确归属的业务场景，发布前建议调整到正式业务域。",
        commonTables: "",
        contacts: "",
        sortOrder: 9999,
        operator: "",
      ...
      setPreprocessMeta("已创建默认业务域，可继续保存当前场景。");
    } catch (err) {
      setError(err.message || "创建默认业务域失败，请前往业务域管理手动创建。");
    } finally {
      setDomainLoading(false);
    }
  }
  ...
  function validateSelectedDomainOrThrow(formData = form) {
    ...
    if (!selected) {
      throw new Error("请先选择业务域后再保存当前场景");
    }
    const selectedId = Number(selected);
    const matched = domains.find((item) => Number(item?.id || 0) === selectedId);
    if (!matched) {
      throw new Error("当前业务域无效，请重载业务域后重新选择");
    }
    return selectedId;
  }
  ...
  return (
    <div className="layout">
      ...
      >
          ...
          <div className="row form-row">
            <label htmlFor="sceneDomain">业务域</label>
            <div className="domain-inline-row">
              ...
              >
                <option value="">{domainLoading ? "业务域加载中…" : "请选择业务域"}</option>
                {domainOptions.map((item) => (
                  <option key={item.id} value={item.id}>{item.domainName}</option>
                ))}
              </select>
              <div className="inline-actions">
                <button className="btn btn-ghost" type="button" onClick={handleReloadDomains} disabled={domainLoading}>
                  {domainLoading ? "重载中…" : "重载业务域"}
                </button>
            ...
            {!domainLoadError && !saveReady ? <p className="field-hint">保存和发布前请先选择业务域。</p> : null}
          </div>
