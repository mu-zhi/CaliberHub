  ...
  return (
    <section className="panel home-focus-panel">
      <section className="home-hero" data-animate="1">
        ...
        <h2>先搜索，再进入治理动作</h2>
        <p className="subtle-note">搜索全局资产：场景、业务域、SQL 片段，优先定位可复用内容。</p>
        <div className="home-search-grid">
          ...
          />
          <UiButton className="home-search-btn" onClick={runSearch} disabled={loading} icon={<Search size={16} />}>
            {loading ? "查询中…" : "搜索"}
          </UiButton>
        </div>
        {hotDomains.length > 0 ? (
          <div className="home-hot-domains">
            <span className="subtle-note">热门业务域：</span>
            {hotDomains.map((name) => (
              <button
                key={name}
                type="button"
      ...
      <div className="home-quick-actions">
        <UiButton as={Link} to="/knowledge/import" variant="primary">导入口径文档</UiButton>
      ...
      <div className="home-scene-list" role="table" aria-label={listTitle}>
        <div className="home-scene-row home-scene-row-head" role="row">
          ...
          <span role="columnheader">业务域</span>
        ...
        {rows.length === 0 ? (
          <UiEmptyState
            ...
            icon={<SearchX size={22} strokeWidth={1.9} />}
