...
describe("PublishCenterPage status presentation", () => {
  ...
  it("shows explicit governance failures from publish checks", async () => {
    globalThis.fetch = vi.fn(async (url) => {
      const path = typeof url === "string" ? url : url.toString();
      if (path.includes("/api/scenes")) return mockJsonResponse(SCENES);
      if (path.includes("/api/publish-checks")) {
        return mockJsonResponse({
          publishReady: false,
          items: [{ key: "projection", name: "图谱投影", message: "投影已完成", passed: true }],
          failedRules: [
            {
          ...
          openBlockingGaps: [
            {
              taskCode: "GAP-GOV-1-GR-DICT-001",
              taskTitle: "字典治理对象缺口",
              status: "OPEN",
            },
          ],
    ...
    await waitFor(() => {
      expect(screen.getByText("字典治理对象")).toBeTruthy();
    });

    expect(screen.getByText("阻断级缺口")).toBeTruthy();
    expect(screen.getByText("字典治理对象缺口")).toBeTruthy();
    expect(screen.getByText("失败规则：字典治理对象；阻断缺口：字典治理对象缺口")).toBeTruthy();
  });
