...
DOC09="${DOC_DIR}/09-数据直通车-口径文档现状-零售客户信息查询.sql"
DOC10="${DOC_DIR}/10-数据直通车-口径文档现状-零售客户信息变更.sql"
DOC11="${DOC_DIR}/11-数据直通车-口径文档现状-代发明细查询.sql"
...
if [[ "$VERIFY_MODE" != "strict" && "$VERIFY_MODE" != "elastic" ]]; then
...
evaluate_results() {
  ...
  if [[ "$TOTAL_RUNS" -gt 0 ]]; then
    mapfile -t sorted_latencies < <(sort -n "$latencies_file")
