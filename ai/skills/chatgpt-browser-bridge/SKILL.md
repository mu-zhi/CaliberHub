...
2. `持续盯守` 的结束条件固定为：当前轮次的细节已经讨论清楚，或双方已明确确认“可作为最终稿”，而不是用户再次手动催促。
3. 只有在用户明确说“只读一次”“只帮我发这条”“先不要继续追问”时，才降级成单次读取或单次发送模式。
4. 若当前前台标签页不再是 `chatgpt.com`，但 `Google Chrome（谷歌浏览器）` 中仍存在已打开的 `ChatGPT` 会话标签，则应优先切回该标签后继续盯守，不把误切到其他网页视为会话自然结束。
