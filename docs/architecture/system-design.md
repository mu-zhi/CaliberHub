...
7. 发布检查：执行 `Publish Check API（发布检查接口）`，校验契约可兑现、覆盖完整、策略可执行、推理资产可回溯、证据可回溯、元数据可落位。
8. 灰度准备：生成新 `snapshot_id（快照标识）` 与唯一 `inference_snapshot_id（推理快照标识）`，刷新缓存和索引，并按 `gray_release_scope（灰度发布范围）` 准备灰度生效窗口。
9. 灰度切换：仅对命中灰度范围的请求暴露新快照对，持续比较新旧路由、命中、降级和风险指标。
10. 全量切换与归档：灰度达标后对运行面全量可见；若不达标则按快照对回滚，并写入变更摘要、差异、审批结论、责任人、`trace_id（追踪编号）`。
