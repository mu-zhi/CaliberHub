...
- `engineering/`：项目总览、开发手册、协作工作流、当前交付状态、开发规范、实施门禁、工程约束与工程协作能力说明。
...
4. 已从主方案拆出的场景级正式特性文档与经路线稿明确拆出的专题特性文档：写入 `architecture/features/`，并保持与两份主文档一致。
5. 当前开发进度、下个阶段工作与任务接力入口：写入 `engineering/current-delivery-status.md`。
6. 仓库定位、阅读顺序、前后端模块现状与阶段划分：写入 `engineering/project-overview.md`。
7. 团队开发流程、模型路由、项目级工具入口与维护规则：写入 `engineering/development-manual.md`。
8. 协作协议、开发规范、工程门禁与工程协作能力说明：写入 `engineering/`。
9. 用户手册与操作指南：写入 `user-guide/`。
10. 过程稿与实施计划：写入 `plans/`。
