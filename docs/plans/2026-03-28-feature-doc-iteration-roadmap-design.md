...
| --- | --- | --- | --- | --- |
| 迭代一 | 打通知识生产主链路，先让材料能进入、能建模、能复核 | 无前置迭代，直接承接主方案第 5、6、7 章与前端知识生产台 | 01-06、03a、04a、05a | 已具备来源接入、证据沉淀、资产建模、推理校验、元数据对齐、复核与缺口闭环，并补齐字典、推理资产详情、表间关联三类原子治理能力 |
| 迭代二 | 打通首页分发、发布、运行、审批、数据地图与跨工作台协同 | 依赖迭代一产出的已复核资产 | 07-12、12a、12b | 已具备首页状态分发、发布检查、灰度回滚、知识包生成、审批导出、图谱浏览、监控审计、跨台锁定态跳转，并补齐 `OpenAPI` 与前端消费收口 |
...
| 主方案范围 | 对应特性文档 |
| --- | --- |
| 知识生产链路、来源接入、证据沉淀、资产建模、推理构建、元数据对齐、复核发布前门禁 | 01-07、03a、04a、05a |
| 运行时主线、知识包输出、覆盖判定、策略决策、审批导出、异步任务、错误码与事件 | 08-11 |
...
| `NFR（非功能需求，Non-Functional Requirements）`、延迟测量、可观测性与容量假设 | 11、12a |
| `system-design.md` §15 图谱投影约定与前端图谱消费口径 | 10、11 |
...
1. 先编写迭代一和迭代二的特性文档，确保平台底座与工作台规则先成文。
